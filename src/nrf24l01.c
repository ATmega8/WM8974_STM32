...
void nRF24L01_InterfaceInit(void)
{
	...
	RCC_AHB1PeriphClockCmd(NRF24L01_CE_IRQ_GPIO_CLOCK, ENABLE);
    ...
    GPIO_Init(NRF24L01_GPIO_CE_PORT, &GPIO_InitStructure);
 
    /* NRF24L01  IRQ pin configuration */
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
    GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP;
    GPIO_InitStructure.GPIO_Pin =  NRF24L01_GPIO_IRQ;
    GPIO_Init(NRF24L01_GPIO_IRQ_PORT, &GPIO_InitStructure);
...
{ 
	uint8_t tempreg;
	uint8_t ds = MASK_TX_DS;
	uint8_t rt = MASK_MAX_RT;
	...
	while(GPIO_ReadInputDataBit(NRF24L01_SPI_GPIO, NRF24L01_GPIO_IRQ) == 0)
	{
		nRF24L01_ReadRegister(CONFIG_REG_ADDR, &tempreg, 1);
		
		if((tempreg & MASK_TX_DS) == 1)
		{
			nRF24L01_WriteRegister(W_REG | CONFIG_REG_ADDR, &ds, 1);
			return NRF24L01_OK;
		}
		else if((tempreg & MASK_MAX_RT) == 1)
		{
			nRF24L01_WriteRegister(W_REG | STATUS_REG_ADDR, &rt, 1);
